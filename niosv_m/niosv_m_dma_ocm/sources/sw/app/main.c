...
// Define base addresses for data source and destination in On-Chip Memory (OCM)
...
int main()
{
    ...
    printf("Hello World from NIOSV/m core on SM ! \n");
    printf("****************Run DMA Test - OCM to OCM **************************************** \n");

    // Write test data into source memory
    printf("Writing into onchip memory from NIOS V\n");
    for (j = 0; j < 32; j = j + 4)
    {
        // Write a repeating pattern of data into source memory
        data_in = (j & 0xFFFFFFFF);
        IOWR_32DIRECT(DATA_SOURCE_BASE, j, data_in); // Write 32-bit data to memory at base address with offset
        printf("Addr 0x%lx     Data written 0x%lx\n", (DATA_SOURCE_BASE + j), IORD_32DIRECT(DATA_SOURCE_BASE, j)); // Read back the written value
    }
    printf("Data write to onchip memory completed\n");

    // Open the mSGDMA device
    dev_ptr = alt_msgdma_open(MSGDMA_0_CSR_NAME); // Open the mSGDMA controller
    if (dev_ptr == NULL)
        printf("Could not open mSG-DMA\n");
    else
        printf("DMA is open mSG-DMA\n");

    // Construct the DMA descriptors for data transfer
    if (alt_msgdma_construct_standard_mm_to_mm_descriptor(
            ...
            WRITE_ADDRESS,
            LEN,
            ALTERA_MSGDMA_DESCRIPTOR_CONTROL_PARK_WRITES_MASK) == -EINVAL)
    {
    ...
    status = alt_msgdma_standard_descriptor_async_transfer(dev_ptr, &STDATA_MSGDMA_DESEC); // Initiate asynchronous transfer
    printf("Status is %x\n", status);
    ...
    for (i = 0; i < 32; i = i + 4)
    {
        // Read back data from destination memory
        data_read = IORD_32DIRECT(DATA_DESTINATION_BASE, i); // Read 32-bit data from memory at base address with offset
        ALT_USLEEP(1000);
    ...
    // Read and print the value of System ID from peripheral core
    printf("Print the value of System ID \n");
    sys_id = IORD_ALTERA_AVALON_SYSID_QSYS_ID(SYSID_QSYS_0_BASE); // Read system ID register
